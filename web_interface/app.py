...
class RegForm(Form):
    ...
    password = PasswordField(
        "Пароль:", validators=[InputRequired(), Length(min=8, max=20)]
    )
...
def login():
    ...
    if request.method == "POST":
        if form.validate():
            ...
            if check_user and check_password_hash(
                check_user["password"], request.form["password"]
            ):
...
def save_file_in_database(file_path, user_id, created_time, filename):
    ...
    new_file_path = os.path.join(
        app.root_path, app.config["TEMP_FOLDER"], str(user_id), "imgs", new_filename
    )
    ...
    document = {
        ...
        "path_to_image": os.path.join(
            app.config["TEMP_FOLDER"], str(user_id), "imgs", new_filename
        ),
        "actions": list(),
    }
...
def upload_files():
    ...
    for i, uploaded_file in enumerate(uploaded_files):
        ...
        file_path = os.path.join(
            app.root_path, app.config["TEMP_FOLDER"], str(user_id), "imgs", filename
        )
        ...
        if filename.rsplit(".", 1)[1].lower() == "vrc":
            ...
            extact_folder = (
                os.path.join(
                    app.root_path, app.config["TEMP_FOLDER"], str(user_id), "imgs"
                )
                + "/"
            )
            png_names = data_handler.convert_vrc_to_png(
                filename, extact_folder, extact_folder
            )
            os.remove(file_path)
            for png_name in png_names:
                ...
                save_file_in_database(
                    extact_folder + png_name,
                    user_id,
                    created_time,
                    filename + "; " + png_name,
                )
    ...
    condition = list(
        history.find({"user_id": user_id}, {"created_time": 1, "_id": 0})
        .sort([("_id", -1)])
        .limit(1)
    )
    ...
    for i in range(len(detection_results)):
        ...
        records = list(
            history.find(
                ...
                {"path_to_image": 1, "actions": 1, "_id": 0},
            )
        )
        ...
        results.append(
            {
                ...
                "description": records[i]["actions"][j]["result"],
            }
        )
...
def recognize_weld():
    ...
    for i in range(len(img_paths)):
        ...
        result, operation_time = weld_segmentation_stage(
            img_paths[i], img_paths[i].replace("imgs", "results_welds")
        )
        item = list(history.find({"user_id": user_id, "path_to_image": img_paths[i]}))[
            0
        ]
        row = {
            "id": str(uuid.uuid1()),
        ...
        history.update(
            {"user_id": user_id, "path_to_image": img_paths[i]},
            {"$push": {"actions": row}},
        )
...
def recognize_defects():
    ...
    for i in range(len(img_paths)):
        ...
        print(img_paths[i])
        result, time = defect_segmentation_stage(
            img_paths[i],
            weld_path,
            path_scale_weld,
            path_defect_mask,
            path_results_detection,
        )
        item = list(history.find({"user_id": user_id, "path_to_image": img_paths[i]}))[
            0
        ]
...
def classify_defects():
    ...
    for i in range(len(img_paths)):
        ...
        weld_path = (
            ...
            + "/"
            + str(user_id)
            + "/results_cls_defects/welds/"
            + img_paths[i].split("/")[-1]
        )
        path_scale_weld = weld_path.replace("welds", "scale_welds")
        path_defect_mask = weld_path.replace("welds", "defect_mask")
        path_detection_results = weld_path.replace("welds", "results_detection")
        path_classification_results = weld_path.replace(
            "welds", "results_classification"
        )
        result, time = defect_classification_stage(
            img_paths[i],
            weld_path,
            path_scale_weld,
            path_defect_mask,
