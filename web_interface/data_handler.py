...
class DataHandler:
    ...
    def convert_vrc_to_png(self, vrc_name, extact_folder, path_input_img):
        ...
        for number in range(len(image)):
            ...
            sep_width_height = (
                b"\x80\x04\x00\x00\x80\x04\x00\x00"  # строка 1152 на 1152 -65535
            )
...
def save_png(find_metr, image, number, path_input_img, vrc_name):
    ...
    im = Image.frombytes(
        "I;16", (int(len(image[number][1]) / 1152 / 2), 1152), image[number][1]
    )
    name = (
        vrc_name[:-4]
        + "_"
        ...
        + str(number)
        + "_"
        + str(round(start_metr[0]))
        + "_"
        + str(round(finish_metr[0]))
        + ".png"
    )
