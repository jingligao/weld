...
class DefectClassificator:
    ...
    def check_mask_on_defects(self, path_defect_mask):
        ...
        if (
            white_color == 0
            and black_color == defect_mask.shape[0] * defect_mask.shape[1]
        ):
            return False
        else:
            return True
    ...
    def predict_classes_defects(self, path_scale_weld, path_defect_mask):
        ...
        for prop in props:
            if (
                int(prop.bbox[0] - 7) > 0
                and int(prop.bbox[2] + 7) < 384
                and int(prop.bbox[1] - 7) > 0
                and int(prop.bbox[3] + 7) < 1152
            ):
                bbox = [
                    int(prop.bbox[0] - 7),
                    int(prop.bbox[1] - 7),
            ...
            crop_defect = np.array(
                cv2.resize(crop_defect, (256, 256)), dtype=np.float32
            )
