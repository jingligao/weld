...
colors = {
    "Burnout": (0, 0, 255),
    "Crack": (255, 0, 0),
    "Unfused-undercut": (255, 0, 255),
    "Interception": (255, 166, 0),
    "Lack of fusion": (255, 255, 0),
    "Slags-Pores": (0, 255, 0),
    "Welding break": (0, 191, 255),
    "Visual defect": (255, 192, 203),
}
...
class Postprocessor:
    ...
    def overlay_binary_mask_on_image(self, path_processing_img, path_results_detection):
        ...
        self.scaling_mask[np.where((self.scaling_mask == [0, 0, 0]).all(axis=2))] = [
            255,
            0,
            0,
        ]
        self.scaling_mask[
            np.where((self.scaling_mask == [255, 255, 255]).all(axis=2))
        ] = [0, 255, 255]
        result = cv2.addWeighted(
            self.scaling_mask, self.alpha, processing_image, 1 - self.alpha, 0
        )
    ...
    def paint_bb_and_classes(
        ...
        for i in range(len(results)):
            ...
            sorted_classes = dict(
                sorted(classes.items(), key=operator.itemgetter(1), reverse=True)
            )
            new_bbox = (
                bbox[0] * scale + initial_boundar1 - 3,
                bbox[1] - 3,
                bbox[2] * scale + initial_boundar1 + 3,
                bbox[3] + 3,
            )

            visual_defect = False
            if (
                new_bbox[2] < initial_boundar1 + self.weld_delta
                or new_bbox[0] > initial_boundar2 - self.weld_delta
            ):
                visual_defect = True
            ...
            # paint bb with id
            draw.rectangle(
                ((new_bbox[1], new_bbox[0]), (new_bbox[3], new_bbox[2])),
                outline=prior_color,
                width=4,
            )
            draw.line(
                ((new_bbox[1], new_bbox[2] - 3), (new_bbox[3], new_bbox[2] - 3)),
                fill=second_color,
                width=6,
            )
            legend = str(id)
            ...
            draw.rectangle(
                (new_bbox[1], new_bbox[0] - h, new_bbox[1] + w, new_bbox[0]),
                fill=prior_color,
            )
...
def get_get_colors(sorted_classes):
    ...
    prior_class, second_class = (
        list(sorted_classes.keys())[0],
        list(sorted_classes.keys())[1],
    )
