...
class DefectDetector:
    ...
    def prepare_input(self, path_scale_weld):
        ...
        s1.append(x)
        s1 = np.asarray(s1)
        pyramid = tuple(pyramid_gaussian(x / 255.0, max_layer=2, downscale=2))
        s2.append(pyramid[1] * 255.0)
        s3.append(pyramid[2] * 255.0)
    ...
    def predict_defect_mask(self):
        ...
        request.inputs["input1"].CopyFrom(
            tf.contrib.util.make_tensor_proto(self.data[0], shape=self.data[0].shape)
        )
        request.inputs["input2"].CopyFrom(
            tf.contrib.util.make_tensor_proto(self.data[1], shape=self.data[1].shape)
        )
        request.inputs["input3"].CopyFrom(
            tf.contrib.util.make_tensor_proto(self.data[2], shape=self.data[2].shape)
        )
    ...
    def apply_threshold(self):
        ...
        y = self.probs
        y[y < self.threshold] = 0.0
        y[y >= self.threshold] = 1.0
