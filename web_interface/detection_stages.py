...
def defect_segmentation_stage(
    img_path, weld_path, path_scale_weld, path_defect_mask, path_results_detection
):
    start = time.time()
    weld_segmentator.get_images(img_path)
    weld_segmentator.predict_weld_mask()
    ...
    initial_boundar1, initial_boundar2 = medium_processor.form_weld_area(
        path_scale_weld
    )
...
def defect_classification_stage(
    img_path,
    weld_path,
    path_scale_weld,
    path_defect_mask,
    ...
    if label == "defects are found":
        ...
        if exist_defects:
            defect_classificator.predict_classes_defects(
                path_scale_weld, path_defect_mask
            )

        postprocessor = Postprocessor()
        if exist_defects:
            postprocessor.paint_bb_and_classes(
                img_path,
                path_classification_results,
                defect_classificator.results,
                initial_boundar1,
                initial_boundar2,
            )
            ...
            postprocessor.scale_mask(
                initial_boundar1, initial_boundar2, path_defect_mask
            )
